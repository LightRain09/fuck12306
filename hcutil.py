...
def getstations():
	global stations
	f = open("/Users/ruochenxing/Desktop/projects/fuck12306/stations.txt")
	content = f.read()
	infos = content.split("@")
	for info in infos:
		if info == '':
...
def parsequery1(response, ft=None):
	...
	table = PrettyTable(["车次", "出发车站", "到达车站", "出发时间",
						 "到达时间", "历时时间", "商务座/特等座", "一等座",
						 "二等座", "高级软卧", "软卧", "硬卧",
						 "软座", "硬座", "无座"])
	...
	for info in data["result"]:
		...
		table.add_row([detail[3], querystationname(detail[4]), querystationname(detail[5]), detail[8],
					   detail[9], detail[10], detail[32], detail[31],
					   detail[30], detail[22], detail[23], detail[28],
					   detail[25], detail[29], detail[26]])
		if canbuy == 1:
			if len(detail[0]) > 10:
...
def getseattype(detail, ft=None):
	if detail[7] != u'空' and detail[7] != u'无':
		if ft is None or u"特等座" in ft:
			return u"特等座", "tz"
	if detail[8] != u'空' and detail[8] != u'无':
		if ft is None or u"一等座" in ft:
			return u"一等座", "zy"
	if detail[9] != u'空' and detail[9] != u'无':
		if ft is None or u"二等座" in ft:
			return u"二等座", "ze"
	if detail[10] != u'空' and detail[10] != u'无':
		if ft is None or u"高级软卧" in ft:
			return u"高级软卧", "gr"
	if detail[11] != u'空' and detail[11] != u'无':
		if ft is None or u"软卧" in ft:
			return u"软卧", "rw"
	if detail[12] != u'空' and detail[12] != u'无':
		if ft is None or u"硬卧" in ft:
			return u"硬卧", "yw"
	if detail[13] != u'空' and detail[13] != u'无':
		if ft is None or u"软座" in ft:
			return u"软座", "rz"
	if detail[14] != u'空' and detail[14] != u'无':
		if ft is None or u"硬座" in ft:
			return u"硬座", "yz"
	return u"无座", "ze"
