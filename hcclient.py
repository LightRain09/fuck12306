...
class Client:
	...
	def print_cookie(self):
		print('; '.join(['='.join(item) for item in self._s.cookies.items()]))
	...
	def get_img(self, t=1):
		while True:
			try:
				...
				if t == 0:
					url = BUY_CODE_URL
				imgstream = self._s.get(url, stream=True, headers=c_headers, verify=False, timeout=5)
	...
	def get_html(self, url):
		...
		response = self._s.get(url, headers=c_headers, verify=False, allow_redirects=False)
	...
	def check_user(self):
		...
		check_user_response = self._s.post(url, data=urllib.urlencode(post_data), headers=c_headers, verify=False, timeout=5)
		...
		if not (json_check_user['httpstatus'] == 200 and bool(json_check_user['status']) and bool(json_check_user['data']['flag'])):
			print "=================check user error", json_check_user
			self.print_cookie()
			return False
		else:
			print "=================check user success", json_check_user
			self.print_cookie()
			return True
	...
	def queryorderwaittime(self, token, tourflag):
		...
		if not (json_resp['httpstatus'] == 200 and json_resp['status']):
			...
			try:
				...
				print "queryorderwaittime error", e
	...
	def confirmforqueue(self, token, passenger, seatinfo, leftticketstr, purpose_codes, train_location, key_check_ischange, tour_flag, randcode=None):
		...
		if json_resp['httpstatus'] == 200 and json_resp['status']:
			if json_resp['data']['submitStatus']:
				...
				if "验证码" in json_resp['data']['errMsg']:
					return NEED_CODE
				return FAIL
		else:
	...
	def submitbuyticket(self, canbuy, buyinfo):
		...
		if self.getqueuecount(token, buyinfo['train_date'], train_no, canbuy[1], leftticketstr, purpose_codes, train_location, buyinfo['seat_type'], buyinfo['from_station'][1], buyinfo['to_station'][1],
			...
			return FAIL
		while True:
			f_result = self.confirmforqueue(token, passenger, seatinfo, leftticketstr, purpose_codes, train_location, key_check_ischange, tour_flag, randcode=randcode)
			if f_result == SUCCESS:
				break
			if f_result == NEED_CODE:
				self.get_img(1)
				verifycode = ''
				while not verifycode:
					verifycode = raw_input("请输入验证码:")
		...
		try:
			...
			if count % 10 == 0:
				if not client.check_user():  # 检测用户是否登录
					client.check_user_page("https://kyfw.12306.cn/otn/leftTicket/init")  # 检测用户是否登录
					return NEED_LOGIN
...
if __name__ == "__main__":
	...
	# train_ft 车次
	buy({"train_date": "2018-02-12", "from_station_name": "深圳", "to_station_name": "南昌", "user_idcard": ""}, ft=["硬卧"], train_ft=["K134"])  # , train_ft=["D2322"] "Z182","K1620"
